...
- 备注：
  ...
  - 当前只允许管理员写入，普通登录用户只读。
  - `sort_order` 越小越靠前。
  - 自 `supabase/sql/20260430_merge_mid_lower_back_groups_into_back.sql` 起，动作库一级背部肌群已统一收敛为 `背部`，不再单独维护 `中背`、`下背` 排序。
  - 迁移脚本会把历史 `背部 / 中背 / 下背` 排序合并为 `背部`；同一动作若同时存在多条旧记录，优先保留顺序为：`背部` > `中背` > `下背`。
  - 合并后的新 `sort_order` 会基于保留记录的优先级、旧排序值、创建时间重新连续编号，从 `0` 开始。
...
| `muscle_group` | `text` | 这条顺序配置属于哪个肌群，如“胸部”“背部”。当前背部相关记录统一使用 `背部`。 |
